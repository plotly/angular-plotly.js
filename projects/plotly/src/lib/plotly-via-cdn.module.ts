...
export class PlotlyViaCDNModule {
  ...
  public static plotlyBundleNames: PlotlyBundleName[] = [
    'basic',
    'cartesian',
    'geo',
    'gl3d',
  ...
  public static setPlotlyVersion(version: string): void {
    ...
    if (!isOk) {
      throw new Error(
        `Invalid plotly version. Please set 'latest' or version number (i.e.: 1.4.3)`
      );
    }
  ...
  public static setPlotlyBundle(bundle: PlotlyBundleName): void {
    const isOk =
      bundle === null ||
      PlotlyViaCDNModule.plotlyBundleNames.indexOf(bundle) >= 0;
    if (!isOk) {
      const names = PlotlyViaCDNModule.plotlyBundleNames
        .map((n) => `"${n}"`)
        .join(', ');
      throw new Error(
        `Invalid plotly bundle. Please set to null for full or ${names} for a partial bundle.`
      );
    }
    ...
    const init = () => {
      ...
      const fn = () => {
        ...
        if (plotly) {
          ...
          throw new Error(
            `Error loading plotly.js library from ${src}. Timeout.`
          );
        }
      };
