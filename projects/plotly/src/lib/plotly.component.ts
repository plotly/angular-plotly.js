...
import { Figure } from './plotly.interface';
...
export class PlotlyComponent implements OnInit, OnChanges, OnDestroy, DoCheck {
  ...
  ngOnInit(): void {
    ...
    if (this.click.observers.length > 0) {
      const msg =
        'DEPRECATED: Reconsider using `(plotlyClick)` instead of `(click)` to avoid event conflict. ' +
  ...
  ngDoCheck(): boolean | void {
    ...
    if (this.updateOnDataChange) {
      if (this.dataDiffer) {
        ...
        this.dataDiffer = this.iterableDiffers
          .find(this.data)
          .create(this.dataDifferTrackBy);
      } else {
        ...
        (plotlyInstance) => {
          ...
          this.eventNames.forEach((name) => {
            ...
            plotlyInstance.on(eventName, (data: any) =>
              (this[name] as EventEmitter<void>).emit(data)
            );
          });
  ...
  updatePlot(): Promise<any> {
    ...
    return this.plotly
      .update(this.plotlyInstance, this.data, layout, this.config, this.frames)
      .then(
        () => {
          const figure = this.createFigure();
